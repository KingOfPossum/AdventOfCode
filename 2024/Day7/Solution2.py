                ...
                for i in range(len(operation)):
                    if i == 0:
                        if operation[i] == "+":
                            ...
                            result = int(equation[1][i] + equation[1][i+1])
                            continue
                        result += int(equation[1][i]) * int(equation[1][i+1])
                        continue
                    if operation[i] == "*":
                        ...
                        result = int(str(result) + equation[1][i+1])
                        continue
                    result += int(equation[1][i+1])

                if result == int(equation[0]):
                    fstValidOperation = operation
                    isValid = True
        if isValid:
            print("Equation is Valid!")
            print(f"The first valid operation is: {fstValidOperation}")
            validEquations.append(equation)
            sum += int(equation[0])

    equations = [equation for equation in equations if equation not in validEquations]
